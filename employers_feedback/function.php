<?php
	...
	if($fn == 'getTableData'){
		...
		$status = true;
		$mainData = array();
		$sql = "SELECT * FROM employ_feedback ORDER BY employ_id DESC";
		...
		if ($result->num_rows > 0) {
			...
			while($row = $result->fetch_array()){
				...
				if($employ_id != ''){
					$action_html = '';
					$action_html .= '<a data-bs-toggle="offcanvas" href="#theme-settings-offcanvas" class="action-icon" onClick="editTabledata('.$employ_id.')"> <i class="mdi mdi-eye"></i></a>';
	...
	if($fn == 'editTabledata'){
		...
		$sql = "SELECT * FROM employ_feedback WHERE employ_id = '" .$serial_no. "' ";
		$result = $con->query($sql);
		...
		if ($result->num_rows > 0) {
			...
			while($row = $result->fetch_array()){ 
				$return_array['employ_id'] = $row['employ_id']; 
				$return_array['employ_name'] = $row['employ_name']; 
				$return_array['employ_phone'] = $row['employ_phone'];  
		...
		$return_array['status'] = $status;  
		$return_array['employ_name'] = $employ_name;
