<?php
	...
	if($fn == 'getTableData'){
		...
		$status = true;
		$mainData = array();
		$sql = "SELECT * FROM alumni_feedback ORDER BY alumni_id DESC";
		...
		if ($result->num_rows > 0) {
			...
			while($row = $result->fetch_array()){
				...
				if($alumni_id != ''){
					$action_html = '';
					$action_html .= '<a data-bs-toggle="offcanvas" href="#theme-settings-offcanvas" class="action-icon" onClick="editTabledata('.$alumni_id.')"> <i class="mdi mdi-eye"></i></a>';
	...
	if($fn == 'editTabledata'){
		...
		$sql = "SELECT * FROM alumni_feedback WHERE alumni_id = '" .$serial_no. "' ";
		$result = $con->query($sql);
		...
		if ($result->num_rows > 0) {
			...
			while($row = $result->fetch_array()){ 
				$return_array['alumni_id'] = $row['alumni_id']; 
				$return_array['alumni_name'] = $row['alumni_name']; 
				$alumni_name = $return_array['alumni_name'];
				$return_array['alumni_dob'] = date('d-M-Y', strtotime($row['alumni_dob'])); 
				$return_array['passout_year'] = $row['passout_year']; 
				$return_array['dept_course'] = $row['dept_course']; 
				$return_array['alumni_address'] = $row['alumni_address']; 
				$return_array['alumni_phone'] = $row['alumni_phone']; 
				$return_array['alumni_email'] = $row['alumni_email']; 
				$return_array['alumni_occupation'] = $row['alumni_occupation']; 
				$return_array['ans_1'] = $row['ans_1']; 
				$return_array['ans_2'] = $row['ans_2']; 
				$return_array['ans_3'] = $row['ans_3']; 
				$return_array['motivated_connected'] = $row['motivated_connected']; 
				$return_array['option1'] = $row['option1']; 
				$return_array['option2'] = $row['option2']; 
				$return_array['option3'] = $row['option3']; 
				$return_array['option4'] = $row['option4']; 
				$return_array['option5'] = $row['option5']; 
				$return_array['option6'] = $row['option6']; 
				$return_array['option7'] = $row['option7']; 
		...
		$return_array['status'] = $status;  
		$return_array['alumni_name'] = $alumni_name;
