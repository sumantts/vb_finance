<?php
	...
	if($fn == 'viewDetailsTrans'){
		...
		$sub_c_sub_total = 0;
		$c_subtotal = 0;
		$sql3 = "SELECT * FROM bank_data WHERE co_id = '" .$co_id. "' AND parent_c_id = '" .$category_id. "' AND sub_c_id = '" .$sub_category_id. "' AND trans_date >= '" .$from_date. "' AND trans_date <= '" .$to_date. "' ORDER BY trans_date DESC";
		$result3 = $con->query($sql3);
		...
		if ($result3->num_rows > 0) {
			while($row3 = $result3->fetch_array()){
				...
				$withdrawal_amount = $row3['withdrawal_amount'];
				$deposit_amount = $row3['deposit_amount'];
				$trans_date = $row3['trans_date'];

				$transaction = new stdClass();
				$transaction->b_id = $b_id;
				$transaction->narration = $narration;
				$transaction->trans_date = date('d-m-Y', strtotime($trans_date));
				$transaction->withdrawal_amount = $withdrawal_amount;
				$transaction->deposit_amount = $deposit_amount;
				$transaction->debit_balance = 0;
				$transaction->credit_balance = 0;
		...
		$all_categories = array();
		$return_array['transactions'] = $transactions;
		$return_array['c_subtotal'] = $c_subtotal;
		$return_array['status'] = $status;
    	echo json_encode($return_array);
	}//function end	
